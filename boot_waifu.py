...
uart_B = UART(UART.UART2, 460800, 8, None, 1, timeout = 1000, read_buf_len = 4096)
uart_B.write('start')
#fix
#SERVO
key_threezero = 15 #10
...
def loadfacedata():
    f = open("/sd/facedata/printbinary0.bin","rb")
    with open("/sd/facedata/printbinary0.bin", "rb") as f:
        record_ftrs = [bytearray(f.read())]
...
while(True):
    ...
    if dir_pressed == 1:
        while(menuenabled):
            ...
            imgv = imgv.draw_string(5, 15, "More options", color=(255,250,250), scale=1)
            lcd.display(imgv)
            check_key()
            utime.sleep_ms(80)
            n+=1
    ...
    #uart_B.write(str(objface.similarity.mean()))
    ...
    if len(pixelchange) == 50:
        pixelchange.clear()
    #img.save("/sd/temp/bg.bmp")#find similarity
    ...
    if code:
        for i in code:
            ...
            if max_score > 60 and max_score < 85:
                f = open("/sd/facedata/printbinary1.bin","rb")
                with open("/sd/facedata/printbinary1.bin", "rb") as f:
                    record_ftrs = [bytearray(f.read())]
            for j in range(len(record_ftrs)):
                max_score = kpu.face_compare(record_ftrs[j], feature)
            if max_score > 85: # 如果最大分数大于85， 可以被认定为同一个人 to 76
            ...
            if greetback == True :
                ...
                print('%d days %d hours %d minutes %d seconds'%prntime(abs(time.ticks_diff(scheduled_time, now))))
